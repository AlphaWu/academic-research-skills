...
- **Comment-markup grammar (#613):** comment syntax appearing in reviewer prose is written in inline code (`` `<!--` ``). Inside the dissent span the checker reads a bare `<!--` as opening an HTML comment wherever it appears — mid-line and lazy-continuation indents included, the two shapes the block-position visibility model deliberately did not read (#609/#612) — so a dissent field hidden by any comment aborts as an unparsed occurrence instead of silently granting a trigger-binding exemption the rendered page does not show. An inline-code mention stays ordinary prose.
