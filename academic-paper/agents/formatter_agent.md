...
When any `literature_corpus[].bibliographic_integrity_signals[]` record is
present, render exactly one `Bibliographic Integrity Advisories` section in
`provenance_summary.md`. Validate records against
`shared/contracts/passport/bibliographic_integrity_signal.schema.json`, sort
rows lexically by `signal_id`, and show the signal type, `epistemic_label`,
`check_status`, `finding`, affected citation/claims, named resolver or list,
recorded/check timestamps, version/hash, and freshness. Deterministic resolver
or list observations and heuristic indicators keep their distinct labels.

This is append-only advisory transcription, not policy evaluation. Never mint
a ref-marker token from the canonical array: every v1
`display.marker_token` is null, so any number of records composes in this one
section without overflowing the marker's one-advisory-token slot. Existing
`CONTAMINATED-*` tokens still come only from the legacy contamination carrier
during dual write. Render `not_checked`, `unknown`, and `degraded` as **NOT
CLEAN — UNRESOLVED**; never summarize those states as passed, absent, or
clean. `terminal_policy.eligible: false` cannot trigger refusal or
`HIGH-BLOCK`. The migration/deprecation authority is
